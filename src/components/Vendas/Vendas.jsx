...
import { fmt, fmtQ, today, todayISO, uid, sortByDate } from '../../lib/utils'
...
export default function Vendas() {
  ...
  // ← DEFAULT: today in ISO format for <input type="date">
  const [dataVnd, setDataVnd] = useState(todayISO())
  ...
  function limparForm() {
    setEditId(null)
    setDataVnd(todayISO()) // reset to today
    setCliId(''); setTabela('1'); setPgto('Dinheiro'); setObs('')
    setCart([]); setCartProdId(''); setCartQtd('1'); setCartPreco('')
    setDescTipo('nenhum'); setDescVal('')
  }
  ...
  // Convert ISO date to pt-BR for storage
  ...
  async function confirmarVenda() {
    ...
    if (editId) {
      ...
      await updateDocument('vendas', editId, {
        data: dataBR, itens: cart, total, pgto, tabela,
      ...
      if (pgto === 'Ficha') {
        await addDocument('fichas', {
          vendaId: vendaRef.id, data: dataBR, total, tabela,
        ...
        await addDocument('lancamentos', {
          ...
          val: total, tipo: 'entrada', cat: 'Vendas', data: dataBR, origem: 'Venda',
        })
      ...
      <Card>
        ...
        {editId && (
          <InfoBox color="amber" style={{ marginBottom: 12 }}>
            ✏️ Você está <strong>editando uma venda</strong>. Altere o necessário e salve.
          </InfoBox>
        )}

        <FormRow>
          <FG label="📅 Data da venda" style={{ maxWidth: 160 }}>
